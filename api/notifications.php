...
function build_notification_link(array $n): string {
    ...
    // For residents, relative paths are correct as they are in root
    ...
    if ($type === 'verification_update') {
        return 'profile.php'; // Or where ID verification is
    }
    ...
    if (str_contains($title, 'request') || str_contains($message, 'request')) {
        return 'requests.php';
    }
